...
class VerticalSliceTest(unittest.TestCase):
    def test_demo_case_runs_from_evidence_to_report(self):
        ...
        self.assertEqual(result.claim.extraction_status, "model_extracted")
        indictment = (case_dir / "indictment.txt").read_text(encoding="utf-8")
        self.assertIn(result.claim_locators[0].source_text, indictment)
        ...
        with self.assertRaisesRegex(ValueError, "CLAIM_EXTRACTION_CONFIRMATION_REQUIRED"):
            confirm_transactions(result, ["TX-T001"], reviewer="tester")
        ...
        self.assertEqual([event.step for event in result.audit_events[-2:]], ["human_review", "verification"])
        self.assertTrue(all(event.input_hash for event in result.audit_events[:4]))
        self.assertTrue(all(event.output_hash for event in result.audit_events[:4]))
