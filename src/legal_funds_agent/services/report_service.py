...
def report_to_html(report: dict[str, Any]) -> str:
    ...
    rows = "".join(
        "<tr>" + "".join(f"<td>{html.escape(str(tx.get(key, '')))}</td>" for key in ("transaction_id", "date", "payer_name", "payee_name", "amount", "disposition", "reason_code", "source_row")) + "</tr>"
...
<h2>逐笔复核记录</h2><table><thead><tr><th>交易号</th><th>日期</th><th>付款人</th><th>收款人</th><th>金额</th><th>处置</th><th>理由</th><th>来源行</th></tr></thead><tbody>{rows}</tbody></table></body></html>"""
